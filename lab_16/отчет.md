...
функция произведения двух квадратных матриц A и B
размером NxN (на выходе матрица C) для потока с колонки start до колонки end.
```c
...
распределение перемножения между потоками 
...
из графика можно сделать вывод, что невсегда большее количество потоков будут работать эффективнее
![alt text](img/5.png)
