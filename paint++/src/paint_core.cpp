...
void drawBrush(Canvas& canvas, int x, int y, const sf::Color& color, int radius) {
    ...
    for (int i = -radius; i <= radius;i++){
        for (int j = -radius; j<=radius;j++){
            if(i*i + j*j <= radius*radius){drawPixel(canvas,i+x,j+y,color);}
        }
    }    
}
        ...
        for (int x = 1; x < WIDTH - 1; ++x) {
            ...
            for (int ky = -1; ky <= 1; ++ky) {
                for (int kx = -1; kx <= 1; ++kx) {
                    const sf::Color& n = original[y + ky][x + kx];
                    totalR += n.r;
                    totalG += n.g;
                    totalB += n.b;
                }
            ...
            int avgG = totalG / 9;
            int avgB = totalB / 9; 
            canvas[y][x] = sf::Color(avgR, avgG, avgB / 1, canvas[y][x].a); 
        }
    }
}
