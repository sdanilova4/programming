...
void xor_encrypt(char *text, char *key, char *output, int len) {
    ...
    output[len] = '\0'; //нулевой байт для конца текста
}
...
void vigenere_encrypt(char *text, char *keyword, char *output) {
    ...
    for (int i = 0; i < text_len; i++) {
        if (isalpha(text[i])) {
            ...
            int shift = tolower(keyword[key_index % key_len]) - 'a'; // отрицательный сдвиг
            output[i] = ((text[i] - base + shift + 26) % 26) + base;
            key_index++;
        } else {
            output[i] = text[i];
        }
...
unsigned int simple_hash(char *str) {
    ...
    for (int i = 0; i < strlen(str); i++) { //был sizeof(str)
